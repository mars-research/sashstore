...
impl<V: Clone> Clone for HashMapItem<V> {
    fn clone(&self) -> Self {
        match self {
            ...
            HashMapItem::Entry { key, ref value } => HashMapItem::Entry {
                key: *key,
                value: value.clone(),
            },
        }
...
impl<V> ResizingHashMap<V> {
        ...
        if cfg!(debug_assertions) {
            assert_eq!(
                underlying
                    .storage
                    .iter()
                    .filter(|s| if let HashMapItem::Entry { .. } = s {
                        true
                    } else {
                        false
                    })
    ...
    // veribetrfs-style iterator (no reference to the map in the iterator object)
    pub fn veribetrfs_iter_inc<'a, 'b>(
        &'a self,
        it: VeribetrfsHashMapIter<'b, V>,
    ) -> VeribetrfsHashMapIter<'a, V> {
...
impl<'a, V: 'a> Iterator for HashMapIterMut<'a, V> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        while self.slot_idx < self.map.underlying.storage.len() {
            let cur_slot = self.slot_idx;
            self.slot_idx += 1;
            if let HashMapItem::Entry { key, ref mut value } =
                &mut self.map.underlying.storage[cur_slot]
            {
                unsafe {
...
impl<'a, V: 'a> HashMapIterOneMut<'a, V> {
    ...
    pub fn next<'b>(&'b mut self) -> Option<(u64, &'b mut V)>
    where
        'a: 'b,
    {
        let mut found = None;
        ...
        if let Some(slot_idx) = found {
            if let HashMapItem::Entry { key, ref mut value } =
                &mut self.map.underlying.storage[slot_idx]
            {
                Some((*key, value))
            } else {
                unreachable!()
